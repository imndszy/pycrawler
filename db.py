...
def check_connection(conn):
    try:
        ...
        logging.warn("error msg : connect error {}".format(str(conn_error.args)))
        return False
...
class _LazyConnection(object):
    ...
    def cursor(self):
        if self.connection is None:
            ...
            while not check_connection(self.connection):
                time.sleep(1)
...
if __name__ == '__main__':
    create_engine('root', '123456', 'njucrawler', 'localhost', charset='utf8')
    def select_test():
        r = select("select *from all_url limit 10")
        time.sleep(5)
        print(threading.currentThread().getName(), len(r))
    for i in range(5):
        t = threading.Thread(target=select_test)
        t.start()
